...
class SodaSelector(ctk.CTk):
    ...
    def configure_layout(self):
        ...
        self.cart_button = ctk.CTkButton(self, text="🛒 View Cart (0)", font=("Helvetica", 14), command=self.view_cart_popup)
        self.cart_button.grid(row=0, column=0, padx=20, pady=10, sticky="ne")
    ...
    def toggle_detection(self):
        if self.is_detecting:
            ...
            try:
                ...
                def aisle_navigation():
                    while self.navigation_active and self.current_aisle <= 3:
                        print(f"Navigating to aisle {self.current_aisle}")
                        self.label_text.set(f"Navigating to aisle {self.current_aisle}")
                        time.sleep(3)
                        self.current_aisle += 1
                    self.navigation_active = False
                    stop()
                ...
                self.navigation_thread = threading.Thread(target=aisle_navigation, daemon=True)
    ...
    def detect_objects(self):
        while self.is_detecting and not self.detection_stopped.is_set():
            try:
                ret, frame = self.cap.read()
                if not ret:
                    self.is_detecting = False
    ...
    def show_summary_popup(self):
        ...
        ctk.CTkLabel(popup, text="Robot search complete.", font=("Helvetica", 18, "bold")).pack(pady=20)
        if self.cart:
            ctk.CTkLabel(popup, text="Items not found:", font=("Helvetica", 14)).pack(pady=10)
            for item in self.cart:
                ctk.CTkLabel(popup, text=item, font=("Helvetica", 14)).pack()
        else:
            ctk.CTkLabel(popup, text="All items have been found!", font=("Helvetica", 14)).pack()
