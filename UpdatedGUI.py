...
import threading
import time
import sys  # Import sys for more robust error handling
...
class SodaSelector(ctk.CTk):
    def __init__(self):
        ...
        try:
            ...
            self.label_text.set(f"Camera Error: {e}")  # Update GUI with error
            ...
            if self.cap and not self.cap.isOpened():
                ...
                sys.exit(1) # Exit if camera init failed
    ...
    def toggle_detection(self):
        if self.is_detecting:
            ...
            try:
                ...
                self.label_text.set(f"Error starting detection: {e}")  # Update GUI
                self.is_detecting = False # Ensure is_detecting is false
                self.start_robot_button.configure(text="Start Robot Vision") # Correct button state.
    ...
    def _attempt_camera_reconnect(self):
        ...
        try:
            ...
            self.label_text.set(f"Camera Reconnect Error: {e}") # Update GUI
            return False
    ...
    def update_camera_feed(self):
        try:
            ...
            if ret:
                ...
                ctk_image = CTkImage(light_image=pil_image, dark_image=pil_image, size=pil_image.size)
                self.camera_frame.configure(image=ctk_image)
            else:
            ...
            try:
                ...
                for box in results.boxes:
                    ...
                    if conf > self.confidence_threshold:
                        ...
                        if label in self.cart and label not in detected_in_frame:
                            ...
                            try:
                                ...
                                self.label_text.set(f"Error removing item: {e}") # send error to GUI
                ...
                pil_image = Image.fromarray(rgb_detection_frame)
                ctk_image = CTkImage(light_image=pil_image, dark_image=pil_image, size=pil_image.size)
                self.camera_frame.configure(image=ctk_image)
